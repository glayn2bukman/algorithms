/*
    ...
    Also, note that becuase we blocks are swapped instead of individual bytes, endianess is NOT
...
void reverse(void *arr, size_t unit_size, size_t count){
    ...
    // unti_size is the size of each item in the array (sizeof(*arr) when calling the function)
    ...
    // for this reason, DONT include the '\0' in <count>
    //
    ...
    // <count> is computed as sizeof(names)/sizeof(*names) when the variable is a pointer and
